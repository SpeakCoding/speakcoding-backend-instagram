...
RSpec.describe 'CommentsController', type: :request do
  it 'should create comments' do
    ...
    post '/comments.json', params: { comment: { body: 'Test 1', post_id: post.id } }, headers: { "Authentication-Token": user.authentication_token }
    expect(response.code.to_i).to eq(200)
    expect(response.parsed_body['data']['body']).to eq('Test 1')
    expect(Comment.count).to eq(1)

    post '/comments.json', params: { comment: { body: 'Test 2', post_id: post.id } }, headers: { "Authentication-Token": user.authentication_token }
    expect(response.code.to_i).to eq(200)
    ...
    expect(response.parsed_body['data']['comments'][0]['body']).to eq('Test 1')
    expect(response.parsed_body['data']['comments'][1]['body']).to eq('Test 2')
  end
end
