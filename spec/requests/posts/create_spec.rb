...
RSpec.describe 'PostsController', type: :request do
  it 'should create post' do
    ...
    post '/posts.json', params: { post: { caption: 'a', location: 'b', image: uri_scheme_encode_image('images/lenna.png') } }, headers: { "Authentication-Token": user.authentication_token }
    expect(response.code.to_i).to eq(200)
    expect(response.parsed_body['data']['caption']).to eq('a')
    expect(response.parsed_body['data']['location']).to eq('b')
  end

  it "shouldn't create post w/o image" do
    user = User.create(email: 'alx.gsv@gmail.com', password: SecureRandom.uuid)
    post '/posts.json', params: { post: { caption: 'a', location: 'b' } }, headers: { "Authentication-Token": user.authentication_token }
    expect(response.code.to_i).to eq(422)
  end

  it "shouldn't create post w/o user" do
    User.create(email: 'alx.gsv@gmail.com', password: SecureRandom.uuid)
    post '/posts.json', params: { post: { caption: 'a', location: 'b', image: uri_scheme_encode_image('images/lenna.png') } }
    expect(response.code.to_i).to eq(403)
    post '/posts.json', params: { post: { caption: 'a', location: 'b', image: uri_scheme_encode_image('images/lenna.png') } }, headers: { "Authentication-Token": '123' }
    expect(response.code.to_i).to eq(403)
  end
