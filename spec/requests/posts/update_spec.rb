...
RSpec.describe 'PostsController', type: :request do
  it 'should update post' do
    ...
    put "/posts/#{post.id}.json", params: { post: { caption: 'c', tags: new_tags } }, headers: { "Authentication-Token": user1.authentication_token }
    expect(response.code.to_i).to eq(200)
    expect(response.parsed_body['data']['caption']).to eq('c')
